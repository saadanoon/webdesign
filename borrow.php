...
<?php
 ...
 $dbb=new PDO('mysql:host=localhost;dbname=library','root','');
  ...
  $rep=$dbb->query("select * from borrow where ID='" . $_SESSION['user']['ID'] . "'");
  echo "<center><div id='don'>";
  $vrac=$rep->fetch();
    if($vrac==null){
	  $rep=$dbb->prepare("insert into borrow values (?,?,NOW(),DATE_ADD(NOW(),INTERVAL 7 DAY))");
	  if ($rep->execute(array($_SESSION['user']['ID'],$_POST['hidisbn']))==true) {
	    $rep1=$dbb->query("select * from book where ISBN='" . $_POST['hidisbn'] . "'");
		$vrac1=$rep1->fetch();
	    echo '<h3>Receipt </h3>';
		echo '<p>Member : ' . $_SESSION['user']['firstname'] . ' ' . $_SESSION['user']['lastname'] . '</p>';
